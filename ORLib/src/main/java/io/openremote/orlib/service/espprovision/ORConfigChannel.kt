...
class ORConfigChannel(private val device: ESPDevice) {
  ...
  suspend fun sendOpenRemoteConfig(
    ...
    val config =
      Request.OpenRemoteConfig.newBuilder()
    ...
    val request =
      Request.newBuilder().setOpenRemoteConfig(config).setId(messageId++.toString()).build()

    val response = sendRequest(request)
    if (
      !response.hasOpenRemoteConfig() ||
        response.openRemoteConfig.status != Response.OpenRemoteConfig.Status.SUCCESS
    ) {
        ...
        object : ResponseListener {
          override fun onSuccess(returnData: ByteArray?) {
            ...
            if (response.id != request.id) {
              ...
              !response.hasResult() ||
                response.result.result != Response.ResponseResult.Result.SUCCESS
            ) {
