...
open class OrMainActivity : Activity() {
  ...
  private val LOG = Logger.getLogger(OrMainActivity::class.java.name)
  ...
  private var onDownloadCompleteReceiver: BroadcastReceiver =
    object : BroadcastReceiver() {
      override fun onReceive(ctxt: Context, intent: Intent) {
        val action = intent.action
        if (DownloadManager.ACTION_DOWNLOAD_COMPLETE == action) {
          Toast.makeText(applicationContext, R.string.download_completed, Toast.LENGTH_LONG).show()
        }
      }
    }
      ...
      webViewClient =
        object : WebViewClient() {
          override fun onReceivedHttpError(
            ...
            // When initialising Keycloak with an invalid offline refresh token (e.g. wrong nonce
            // because
            ...
            if (
              request.url.lastPathSegment != null &&
                request.url.lastPathSegment == "token" &&
                request.method == "POST" &&
          ...
          override fun onReceivedError(
            ...
            // Remote debugging session from Chrome wants to load about:blank and then fails with
            // "ERROR_UNSUPPORTED_SCHEME", ignore
            if ("net::ERR_CACHE_MISS".contentEquals(error.description)) {
              return
            }
            if (
              request.url.toString() == "about:blank" && error.errorCode == ERROR_UNSUPPORTED_SCHEME
            ) {
              return
            }
            handleError(
  ...
  override fun onRequestPermissionsResult(
    ...
    if (requestCode == ORConstants.WRITE_PERMISSION_FOR_DOWNLOAD) {
      ...
      requestCode == BleProvider.BLUETOOTH_PERMISSION_REQUEST_CODE ||
        requestCode == BleProvider.ENABLE_BLUETOOTH_REQUEST_CODE
    ) {
      ...
      requestCode == ESPProvisionProvider.BLUETOOTH_PERMISSION_ESPPROVISION_REQUEST_CODE ||
        requestCode == ESPProvisionProvider.ENABLE_BLUETOOTH_ESPPROVISION_REQUEST_CODE
    ) {
  ...
  private inner class WebAppInterface(private val activity: Activity) {
    @JavascriptInterface
    ...
    private fun handleQrScannerProviderMessage(data: JSONObject) {
      ...
      when {
        ...
        action.equals("PROVIDER_ENABLE", ignoreCase = true) -> {
          ...
          qrScannerProvider?.enable(
            object : QrScannerProvider.ScannerCallback {
              override fun accept(responseData: Map<String, Any>) {
                notifyClient(responseData)
              }
            }
        ...
        action.equals("SCAN_QR", ignoreCase = true) -> {
          qrScannerProvider?.startScanner(
            this@OrMainActivity,
            object : QrScannerProvider.ScannerCallback {
              override fun accept(responseData: Map<String, Any>) {
                notifyClient(responseData)
    ...
    private fun handleESPProvisionProviderMessage(data: JSONObject) {
      ...
      when {
        ...
        action.equals(ESPProvisionProviderActions.START_BLE_SCAN, ignoreCase = true) -> {
          // Must define a value for prefix, to ensure scan is started in case we need to request
          // BLE permissions (though it should not happen here)
        ...
        action.equals(ESPProvisionProviderActions.PROVISION_DEVICE) -> {
          ...
          if (!userToken.isNullOrEmpty()) {
            if (baseUrl != null) {
              ...
              espProvisionProvider?.provisionDevice(
                getESPProvisionApiURL(baseUrl!!, realm),
                userToken,
                realm,
              )
            } else {
    ...
    private fun getESPProvisionRealm(): String {
      return baseUrl
        ...
        ?: sharedPreferences.getString(ORConstants.REALM_KEY, null)?.takeIf { it.isNotBlank() }
        ?: "master"
    }
