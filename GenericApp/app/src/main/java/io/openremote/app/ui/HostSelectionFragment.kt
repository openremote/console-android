...
/**
 * A simple [Fragment] subclass. Use the [HostSelectionFragment.newInstance] factory method to
...
class HostSelectionFragment : Fragment() {
    ...
    parentActivity.apiManager.getConsoleConfig { statusCode, consoleConfig, error ->
      when (statusCode) {
        ...
        404 -> { // no console_config.json
                ...
                if (appInfoMap?.isNotEmpty() == true) {
                  ...
                  parentActivity.apiManager.getApps { statusCode, apps, error ->
                    if (apps?.isNotEmpty() == true) {
                      ...
                      // TODO remove inserting manager app after all manager instances are updated
                      // with console_settings_improvement branch
          ...
          parentActivity.runOnUiThread {
            binding.errorView.visibility = View.VISIBLE
            binding.errorTextView.text = resources.getText(R.string.could_not_connect_to_domain)
          }
        }
      }
    }
  ...
  companion object {
    @JvmStatic fun newInstance() = HostSelectionFragment()
  }
}
