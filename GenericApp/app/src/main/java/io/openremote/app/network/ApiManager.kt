...
class ApiManager(private val baseUrl: String) {
  ...
  fun createUrlRequest(
    ...
    if (queryParameters != null) {
      for ((queryParameterKey, queryParameterValue) in queryParameters) builder
      ...
      val parsedResult =
        try {
          if (this.responseCode in 200..299) {
            Triple(
              this.responseCode,
              mapper.readValue<T>(this.inputStream.bufferedReader().readText()),
              null,
            )
          } else {
  ...
  private fun HttpURLConnection.writeBody(item: Any) {
    ...
    outputWriter.write(mapper.writeValueAsString(item))
    outputWriter.flush()
  }
}
